...
def process_folder(folder: str, port: str, baud: int = 115200):
    ...
    for filepath in files:
        ...
        print(f"  Encoded size: {len(packet)} bytes")
...
if __name__ == '__main__':
    ...
    for filepath in files:
        patient = parse_patient_file(filepath)
        packet  = encode_patient(patient)
        print(f"  {os.path.basename(filepath)}  ({len(packet)} bytes)")

        if port:
            print(f"  >>> Place RFID tag on reader then press Enter...")
            input()
